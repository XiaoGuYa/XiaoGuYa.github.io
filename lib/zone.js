...
function indexTalk() {
    ...
    function talk(ls) {
        let html = ''
        ls.forEach((item, i) => { html += `<li class="item li-style item-${i + 1}">${item}</li>` });
    ...
    if (d) talk(d);
        ...
        fetch('https://memos-on-replit.xiaoguya.repl.co/api/memo?creatorId=1&tag=说说&limit=10').then(res => res.json()).then(data => { // 更改地址和ID
